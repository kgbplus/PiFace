...
def P0():	# Process 0 controlles servo0
	...
	_Servo0CP = 135		# by making the current position and desired position unequal,-
	_Servo0DP = 140		# 	we can be sure we know where the servo really is. (or will be soon)
	while True:
		time.sleep(speed)
		...
		if not Servo0S.empty():			# Constantly read read Servo0S in case the main process-
			...
			speed = .01 / _Servo0S		# 	the wait between loops will be, so the servo moves faster
		if _Servo0CP < _Servo0DP:					# if Servo0CP less than Servo0DP
			...
			ServoBlaster.write('5=' + str(_Servo0CP) + '\n')#
			print '5=' + str(_Servo0CP)
			ServoBlaster.flush()				#
...
def P1():	# Process 1 controlles servo 1 using same logic as above
	...
	while True:
		...
		if _Servo1CP < _Servo1DP:
			_Servo1CP += 1
			Servo1CP.put(_Servo1CP)
			ServoBlaster.write('2=' + str(_Servo1CP) + '\n')
			print '2=' + str(_Servo1CP)
			ServoBlaster.flush()
...
while True:
	...
	if not faceFound:		# if no face was found...-
		...
		nofacecount += 1	# time from last face recognition


	x,y,w,h = face
	Cface = [(w/2+x),(h/2+y)]	# we are given an x,y corner point and a width and height, we need the center
	print str(Cface[0]) + "," + str(Cface[1]) + "   nofacecount = " + str(nofacecount)
